...
aws s3api put-bucket-acl --bucket shared --grant-read uri=http://acs.amazonaws.com/groups/global/AuthenticatedUsers --grant-write uri=http://acs.amazonaws.com/groups/global/AuthenticatedUsers --endpoint http://localhost:8010
aws s3api put-bucket-policy --bucket demobucket --endpoint http://localhost:8010 --policy '{"Statement": [{"Action": ["s3:GetObject"],"Effect": "Allow","Principal": "*","Resource": ["arn:aws:s3:::*"]}],"Version": "2012-10-17"}'
aws s3api put-bucket-policy --bucket home --endpoint http://localhost:8010 --policy '{"Statement": [{"Action": ["s3:GetObject"],"Effect": "Allow","Principal": "*","Resource": ["arn:aws:s3:::*"]}],"Version": "2012-10-17"}'
aws s3api put-bucket-policy --bucket shared --endpoint http://localhost:8010 --policy '{"Statement": [{"Action": ["s3:GetObject"],"Effect": "Allow","Principal": "*","Resource": ["arn:aws:s3:::*"]}],"Version": "2012-10-17"}'
