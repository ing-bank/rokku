...
trait LineageHelpers extends EventProducer {
  ...
  def deleteEntityLineage(entityName: String, userSTS: User, entityType: String = AWS_S3_OBJECT_TYPE)(
  ...
  def createSingleEntity(entityName: String, userSTS: User, entityValues: JsObject)(
    implicit id: RequestId
  ): Future[Done] = {
    logger.debug(s"Creating lineage for request, $entityName at $timestamp")
    sendSingleMessage(prepareEntityFullCreateMessage(userSTS, Vector(entityValues)).toString, ATLAS_HOOK_TOPIC)
  }
  ...
  def readOrWriteLineage(
  ...
  )(implicit id: RequestId): Future[Done] = {
    ...
    val s3ObjectEntityJs =
      s3ObjectEntity(bucketObject, pseudoDir, guids.pseudoDir, userName, lh.contentType.toString(), guids.objectGuid)
        ...
        sendSingleMessage(
          prepareEntityFullCreateMessage(
            ...
            Vector(
              ...
              pseudoDirEntityJs,
              s3ObjectEntityJs,
              externalPathEntityJs,
              processEntity(
                s"${clientType}_$timestamp",
                userName,
                method.rangerName,
                clientHost,
                guids.serverGuid,
  ...
  def lineageForCopyOperation(
    ...
    val srcPseudoDirEntityJs =
      pseudoDirEntity(pseudoDirFromCopySrc, bucketNameFromCopySrc, srcGuids.bucketGuid, userName, srcGuids.pseudoDir)
    val srcS3ObjectEntityJs = s3ObjectEntity(
      objectNameFromCopySrc,
      pseudoDirFromCopySrc,
      srcGuids.pseudoDir,
      userName,
      lh.contentType.toString(),
      srcGuids.objectGuid
    )
    ...
    val destS3ObjectEntityJs = s3ObjectEntity(
      bucketObject,
      pseudoDir,
      destGuids.pseudoDir,
      userName,
      lh.contentType.toString(),
      destGuids.objectGuid
    )
    val copyProcessEntityJs = processEntity(
      s"${clientType}_$timestamp",
      userName,
      method.rangerName,
      clientHost,
      ...
      sendSingleMessage(
        prepareEntityFullCreateMessage(
          ...
          Vector(
            serverEntityJs,
            srcBucketEntityJs,
            srcPseudoDirEntityJs,
            srcS3ObjectEntityJs,
            destPseudoDirEntityJs,
            destS3ObjectEntityJs,
            copyProcessEntityJs
          )
        ).toString(),
        ATLAS_HOOK_TOPIC
