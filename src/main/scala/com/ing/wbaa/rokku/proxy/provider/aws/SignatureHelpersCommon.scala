...
trait SignatureHelpersCommon {
  ...
  def getSignableRequest(httpRequest: HttpRequest, request: DefaultRequest[_] = new DefaultRequest("s3"))(
...
object SignatureHelpersCommon {
  ...
  val awsVersion: HttpRequest => SignatureHelpersCommon = { implicit request =>
    extractHeaderOption("authorization")
      .map {
