...
trait PostRequestActions {
  ...
  protected[this] def createLineageFromRequest(httpRequest: HttpRequest, userSTS: User, clientIPAddress: RemoteAddress)(
    implicit id: RequestId
  ): Future[Done]

  protected[this] def emitEvent(s3Request: S3Request, method: HttpMethod, principalId: String)(
    implicit id: RequestId
  ): Future[Done]

  protected[this] def setDefaultBucketAcl(bucketName: String): Future[Unit]

  private[this] def createAtlasLineage(
  ...
  private[this] def createBucketNotification(
    response: HttpResponse,
    httpRequest: HttpRequest,
    s3Request: S3Request,
    userSTS: User
    ...
    httpRequest.method match {
      case HttpMethods.POST | HttpMethods.PUT | HttpMethods.DELETE
          if bucketNotificationEnabled && (response.status == StatusCodes.OK || response.status == StatusCodes.NoContent) =>
