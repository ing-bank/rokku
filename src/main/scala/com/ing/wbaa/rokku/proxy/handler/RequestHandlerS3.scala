...
import com.ing.wbaa.rokku.proxy.handler.parsers.RequestParser.ReadRequestType
...
trait RequestHandlerS3 extends RadosGatewayHandler with S3Client with UserRequestQueue with RequestParser {
  ...
  protected[this] def executeRequest(request: HttpRequest, userSTS: User, s3request: S3Request)(implicit id: RequestId): Future[HttpResponse] = {
    ...
    val isCacheRequest = awsRequestFromRequest(request).isInstanceOf[ReadRequestType] && storageS3Settings.storageCacheEnabled

    def newRequest(cacheRequest: Boolean): HttpRequest = cacheRequest match {
      case true =>
        request
          .withUri(storageS3Settings.storageCacheURL)
          .removeHeader(headerName = "Raw-Request-URI")
          .withHeaders(`Raw-Request-URI`.apply(storageS3Settings.cacheAPIPath + request.uri.path + "?" + request.uri.rawQueryString.getOrElse("")))
    ...
    fireRequestToS3(newRequest(isCacheRequest), userSTS, isCacheRequest).flatMap { response =>
      if (response.status == StatusCodes.Forbidden && handleUserCreationRadosGw(userSTS))
        fireRequestToS3(newRequest(isCacheRequest), userSTS, isCacheRequest).flatMap(retryResponse => Future(filterResponse(request, userSTS, s3request, retryResponse)))
      else if (response.status == StatusCodes.NotFound) {
        logger.info("Failed to get s3 object from cache, sending to Ceph directly...")
        fireRequestToS3(newRequest(false), userSTS, false).flatMap(retryResponse => Future(filterResponse(request, userSTS, s3request, retryResponse)))
      } else {
   ...
   */
  protected[this] def fireRequestToS3(request: HttpRequest, isCacheRequest: Boolean)(implicit id: RequestId): Future[HttpResponse] = {
    isCacheRequest match {
      case true  => logger.info(s"Request sent to Cache: {}", request)
      case false => logger.info(s"Request sent to Ceph: {}", request)
    }

    Http()
      .singleRequest(request)
      .andThen { case Success(r) =>
        isCacheRequest match {
          case true    => logger.info(s"Received response from Cache: {}", r.status)
          case false   => logger.info(s"Received response from Ceph: {}", r.status)
        }
      }
   ...
   */
  protected[this] def fireRequestToS3(request: HttpRequest, user: User, isCacheRequest: Boolean)(implicit id: RequestId): Future[HttpResponse] = {
    if (storageS3Settings.isRequestUserQueueEnabled) {
      if (addIfAllowedUserToRequestQueue(user)) {
        fireRequestToS3(request, isCacheRequest).andThen { case _ => decrement(user) }
      } else {
      ...
      fireRequestToS3(request, isCacheRequest)
    }
  }
}
