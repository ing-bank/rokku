...
trait RadosGatewayHandler extends LazyLogging {
  ...
  protected[this] def handleUserCreationRadosGw(userSTS: User): Boolean = {
    val userOnCeph = getUserOnCeph(userSTS.userName)

    userOnCeph.map(_.credentials) match {
      ...
      case Some(creds) if creds == List(CredentialsOnCeph(userSTS.accessKey, userSTS.secretKey)) => false
