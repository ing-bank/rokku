  ...
  "KafkaMessageProvider" should {
    "Send message to correct topic with Put or Post" in {
      ...
      withRunningKafka {
        ...
        emitEvent(s3Request, HttpMethods.PUT, "testUser", AWSRequest(RequestUnknown()))
    ...
    "Send message to correct topic with Delete" in {
      ...
      withRunningKafka {
        ...
        emitEvent(s3Request, HttpMethods.DELETE, "testUser", AWSRequest(RequestUnknown()))
        assert(consumeFirstStringMessageFrom(deleteEventsTopic).contains("s3:ObjectRemoved:DELETE"))
      }
    }

    "fail on incomplete data" in {
      recoverToSucceededIf[Exception](emitEvent(s3Request.copy(s3Object = None), HttpMethods.PUT, "testUser", AWSRequest(RequestUnknown())))
    }
  }
