...
import com.ing.wbaa.gargoyle.proxy.provider.{AuthenticationProviderSTS, LineageProviderAtlas, SignatureProviderAws}
...
class GargoyleS3ProxyItTest extends AsyncWordSpec with DiagrammedAssertions
  ...
  def withSdkToMockProxy(testCode: (AWSSecurityTokenService, Authority) => Future[Assertion]): Future[Assertion] = {
    val proxy: GargoyleS3Proxy = new GargoyleS3Proxy with RequestHandlerS3 with AuthenticationProviderSTS with LineageProviderAtlas with SignatureProviderAws {
